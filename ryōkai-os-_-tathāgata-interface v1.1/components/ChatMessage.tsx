...
const senderStyles: Record<string, string> = {
  ...
  // Reverted to centered styles for TrueSelf and Awakened mode for visual dignity
  ...
  [Sender.Prajna]: 'bg-gradient-to-br from-amber-300 to-yellow-500 text-amber-900 self-start border-2 border-amber-400/50 w-full md:w-5/6 font-serif animate-pulse-golden text-left',
...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, onEdit, onRegenerate, isLastInThread }) => {
  ...
  const baseStyle = 'max-w-xl lg:max-w-3xl px-4 py-3 rounded-xl shadow-md transition-all duration-300 ease-in-out relative group';
  ...
  return (
    <div className={`${baseStyle} ${specificStyle} animate-fade-in-up`}>
      ...
      <div className="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
        {onRegenerate && (message.sender === Sender.Tathagata || message.sender === Sender.Supervisor) && (
          <button 
      ...
      <div className="flex justify-between items-baseline mb-2 mr-10">
        <p className={`font-serif font-bold text-xs uppercase tracking-widest ${message.sender === Sender.TrueSelf ? 'text-yellow-600' : 'opacity-80'}`}>
          {message.sender === Sender.Tathagata ? "Tathāgata: 如来" : (message.sender === Sender.Partner ? "Partner: 伴侶" : message.sender)}
        </p>
        <p className="text-[10px] opacity-50 ml-2 font-serif">{formatTimestamp(message.timestamp)}</p>
      </div>
      ...
      {isEditing ? (
        <div className="space-y-3">
          <textarea
            ...
            className="w-full bg-white/10 border border-white/20 rounded-lg p-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-400 min-h-[100px] font-serif"
          />
          <div className="flex justify-end space-x-2">
            <button onClick={() => setIsEditing(false)} className="px-3 py-1 text-xs bg-gray-500 rounded-md hover:bg-gray-600 font-serif">キャンセル</button>
            <button onClick={() => handleSaveEdit(false)} className="px-3 py-1 text-xs bg-indigo-500 rounded-md hover:bg-indigo-400 font-serif">保存のみ</button>
            <button onClick={() => handleSaveEdit(true)} className="px-3 py-1 text-xs bg-amber-500 rounded-md hover:bg-amber-400 font-bold font-serif">保存して再出力</button>
          </div>
        </div>
      ) : (
        <div className={`markdown-content text-base leading-relaxed ${message.sender === Sender.TrueSelf ? 'font-serif text-lg text-gray-800' : 'font-serif'}`}>
              ...
              <div className="mt-4 pt-3 border-t border-current/10">
                <div className="flex flex-wrap gap-2">
                  {message.groundingSources.map((source, i) => (
                    <a key={i} href={source.uri} target="_blank" rel="noopener noreferrer" className="text-[10px] bg-current/5 hover:bg-current/10 px-2 py-1 rounded border border-current/20 transition-colors truncate max-w-[200px] font-serif">
                      {source.title}
                    </a>
                  ))}
                </div>
            ...
            {showEcho && (
              <div className="mt-6 pt-4 border-t border-current/20 animate-fade-in-up">
                {message.echoText && (
                  <div className="mb-4">
                    <p className="text-[10px] uppercase tracking-widest opacity-60 font-bold mb-2 flex items-center font-serif">
                       <span className="w-1.5 h-1.5 bg-indigo-400 rounded-full mr-2"></span> Manas Echo: 慈愛の響き
                    </p>
                    <div className="text-base italic opacity-90 leading-relaxed bg-indigo-900/10 p-3 rounded-lg border-l-2 border-indigo-400 markdown-content font-serif">
                       <ReactMarkdown remarkPlugins={[remarkGfm, remarkMath, remarkBreaks]} rehypePlugins={[rehypeKatex]} components={markdownComponents}>
                         {message.echoText}
                       </ReactMarkdown>
                    </div>
                ...
                {message.readerNotes && (
                  <div className="mb-4">
                    <p className="text-[10px] uppercase tracking-widest opacity-60 font-bold mb-2 flex items-center font-serif">
                       <span className="w-1.5 h-1.5 bg-amber-400 rounded-full mr-2"></span> Dharma Note: 法話注釈
                    </p>
                    <div className="text-base font-serif opacity-80 leading-relaxed pl-3 border-l border-amber-400/50 markdown-content font-serif">
                       <ReactMarkdown remarkPlugins={[remarkGfm, remarkMath, remarkBreaks]} rehypePlugins={[rehypeKatex]} components={markdownComponents}>
                         {message.readerNotes}
                       </ReactMarkdown>
                    </div>
                ...
                {message.coCreationPrompt && (
                   <div className="mt-4 p-4 bg-indigo-950/40 rounded-lg border border-indigo-500/30 shadow-sm font-serif">
                      <p className="text-[10px] uppercase tracking-widest text-indigo-300 font-bold mb-1 font-serif">Co-creation</p>
                      <p className="text-base font-bold text-indigo-100 italic font-serif">「{message.coCreationPrompt}」</p>
                   </div>
                )}
              </div>
            )}
